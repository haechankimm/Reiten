...
6. **마이그레이션 018 실행** — 상품 사진마다 컬러를 태그해서 컬러 클릭 시 그 사진으로 바뀌게
...
7. **마이그레이션 016(`orders.status` CHECK 제약) 재확인** — REST API로는 제약 존재 여부를
...
8. **재입고 발주 알림 · 정산 자동화를 실제 데이터로 검증** — 재입고 알림(`checkRestockNeeded()`,
   ...
   수동 호출해 확인해보세요.
9. (선택) **가상계좌 결제 · 카카오 알림톡 연동** — 둘 다 우리 쪽 코드만으로는 못 끝내고 외부
