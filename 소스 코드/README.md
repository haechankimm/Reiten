...
1. ~~상품 등록 실패 버그~~ — 2026-08-13에 재보고받아 원인 파악·수정 완료. 실제로는 "상품 등록" 자체가 아니라 상품 등록 폼의 **사진 업로드**가 실패하는 것이었음(사용자가 처음엔 "상품 등록"으로 설명했다가 재확인 후 정정). 원인 두 가지: ① 업로드 실패 시 서버가 구체적인 이유("15MB 이하 이미지만 가능" 등)를 응답해도 클라이언트가 그 내용을 버리고 항상 같은 뭉뚱그린 메시지만 띄워서 사용자가 원인을 알 수 없었음(상품·룩북 사진 둘 다 같은 버그) → 서버 응답의 실제 에러 메시지를 그대로 보여주도록 수정 ② 업로드 실패 시 파일 선택 input이 계속 비활성 상태로 남아 재시도 자체가 안 됐음(재활성화하는 코드가 없었음) → `finally`로 항상 재활성화되게 수정 ③ 사진 용량 제한이 5MB였는데 요즘 휴대폰 사진(특히 아이폰)은 쉽게 이를 넘어서 이게 진짜 원인이었을 가능성이 높음 → 15MB로 상향(상품·룩북·리뷰 사진 공통). Cloudinary 업로드 자체는 실제 키로 테스트해서 정상 동작 확인됨
2. ~~`007`·`008`·`009` 마이그레이션 실행~~ — 2026-08-13에 Supabase에서 전부 실행 완료(직접 조회해서 테이블 존재 확인함)
3. **⚠️ 상품 재고(`inventory`) 초기값을 최소 1개 상품이라도 채우고 배포할 것** — 고객 화면의 품절 표시가 관리자가 체크한 값과 실제 `inventory` 수량을 합쳐서 보여주도록 고쳤는데(아래 "Works 추가 기능(2026-08-12, 2차)" 참고), 지금 Supabase `inventory` 테이블이 사실상 비어 있어서 **이 상태로 배포하면 전 상품·전 사이즈가 품절로 보임**. 관리자 패널 "재고" 탭에서 실제 수량을 입력하기 전까지는 이 패치를 배포하지 말 것(또는 배포와 동시에 재고 입력)
4. **⚠️ 카드결제 — `PORTONE_CHANNEL_KEY`까지 Render에 등록 완료해서 `cardPaymentEnabled: true`로 실제 배포됨(2026-08-13, `https://reiten.kr/api/config`로 직접 확인)**. 단, **지금 등록된 채널은 테스트 채널**이라 실제 카드로 결제해도 진짜 돈이 빠지지 않음(포트원 PG 심사가 아직 안 끝난 상태) — 그래서 지금 카드결제 옵션이 고객 눈에 보이는 건 문제 없지만, **실제 손님에게 카드결제를 안내하기 전에 반드시**: ① [11번](#11-카드결제pg--포트원portone-v2) "켜는 방법" 4번대로 테스트 카드로 결제 1건을 결제창→서버검증→웹훅까지 실제로 통과시켜 확인 ② 포트원 PG 심사가 끝나면 발급되는 **라이브 채널 키로 `PORTONE_CHANNEL_KEY`를 교체**(테스트 채널 키를 그대로 두면 손님이 결제해도 실제로는 돈이 안 빠지는 상태로 계속 남음)
5. **`https://works.reiten.kr`에서 실제 로그인 → 새 UI(사이드바 대시보드)로 주문/재고/상품까지 정상 노출되는지 재확인** — 로컬 검증은 끝났지만 실배포 확인은 아직
6. 위 항목들이 확인되면 → `account.html`의 옛 관리자 패널(`#admin-panel`)과 `server.js`의 `/__works-preview` 임시 라우트 제거(정리 커밋)
7. GitHub 푸시 상태 — Works 독일어 번역 수정(`i18n.js`)까지는 이미 커밋·푸시됨. **통화 표기·버튼 레이아웃·사진 업로드 버그 수정(`server.js`, `works/index.html`)은 아직 커밋 전** — 새 세션 시작 시 `git status`로 확인 후 커밋·푸시할 것
