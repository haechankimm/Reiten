...
0. ~~상품 색상 팔레트를 관리자 패널에서 자유롭게 추가·수정·삭제 + 다크모드 지퍼 발광~~ — 2026-08-13에 구현·커밋(`d2f87ce`)하고 사용자가 직접 `010_product_colors.sql` 마이그레이션 실행 + Works에서 색상 추가·삭제 테스트까지 마침, GitHub 푸시도 완료(origin/main과 동기화 확인됨). `reiten.kr`/`works.reiten.kr` 실배포에서도 콘솔 에러 없이 정상 동작 확인(다크모드 지퍼 발광 포함). 자세한 내용은 아래 "최근 작업 이력" 참고
1. ~~사진 업로드 버그~~ — 원인 두 가지(에러 메시지 뭉뚱그림, 실패 시 input 비활성 고착) 수정 + 용량 제한 5MB→15MB(`f63eaad`). 이후 "1.7MB인데도 실패한다"는 재보고가 있었지만, 알고 보니 그 테스트가 수정 사항이 배포되기 전에 이뤄진 것이었고, 2026-08-13에 사용자가 재확인한 결과 정상 동작 확인 — 더 이상 유효하지 않은 항목이라 제거함
...
4. **⚠️ 상품 재고(`inventory`) 초기값을 최소 1개 상품이라도 채우고 배포할 것** — 고객 화면의 품절 표시가 관리자가 체크한 값과 실제 `inventory` 수량을 합쳐서 보여주도록 고쳤는데(아래 "Works 추가 기능(2026-08-12, 2차)" 참고), 지금 Supabase `inventory` 테이블이 사실상 비어 있어서 **이 상태로 배포하면 전 상품·전 사이즈가 품절로 보임**. 관리자 패널 "재고" 탭에서 실제 수량을 입력하기 전까지는 이 패치를 배포하지 말 것(또는 배포와 동시에 재고 입력). **2026-08-13에 "재고" 탭 화면을 상품코드·사이즈만 나열하던 텍스트 목록에서 상품 썸네일·이름 카드 + 사이즈별 수량(품절=빨강, 2개 이하=주황 강조)으로 개선함**(`works/index.html`의 `paintAdminInventory()`) — 수량을 채우는 작업 자체는 여전히 필요하지만, 이제 어떤 상품·사이즈가 비었는지 한눈에 훑기는 쉬워졌음
5. **⚠️ 카드결제 — `PORTONE_CHANNEL_KEY`까지 Render에 등록 완료해서 `cardPaymentEnabled: true`로 실제 배포됨(2026-08-13, `https://reiten.kr/api/config`로 직접 확인)**. 단, **지금 등록된 채널은 테스트 채널**이라 실제 카드로 결제해도 진짜 돈이 빠지지 않음(포트원 PG 심사가 아직 안 끝난 상태) — 그래서 지금 카드결제 옵션이 고객 눈에 보이는 건 문제 없지만, **실제 손님에게 카드결제를 안내하기 전에 반드시**: ① [11번](#11-카드결제pg--포트원portone-v2) "켜는 방법" 4번대로 테스트 카드로 결제 1건을 결제창→서버검증→웹훅까지 실제로 통과시켜 확인 ② 포트원 PG 심사가 끝나면 발급되는 **라이브 채널 키로 `PORTONE_CHANNEL_KEY`를 교체**(테스트 채널 키를 그대로 두면 손님이 결제해도 실제로는 돈이 안 빠지는 상태로 계속 남음). **PG사 계약 진행 상황(2026-08-13 기준, 포트원 콘솔 확인)**: `KG이니시스`(신용카드 일반결제 — 입점 심사중, 신용카드 정기결제 — PG사 접수완료), `NHN KCP`(신용카드 일반결제 — 카드사 심사 요청/진행중, 휴대폰 본인인증 — PG사 접수완료, 신용카드 정기결제는 계약 취소됨 — Reiten은 정기결제를 안 쓰므로 문제 없음), `카카오페이`(PG사 접수완료). 고객확인이행(KYC — 사업자통장사본·사업자등록증·대표자 신분증) 서류도 제출 완료. **지금은 심사 결과를 기다리는 단계** — PG사 1차 심사 + 카드사 심사(사별로 별도 진행) 총 영업일 기준 1~2주 정도가 일반적이며, 서류 보완 요청이 오면 그때만 대응하면 됨. 승인되면 포트원에서 라이브 채널 키가 발급되니 그때 ②번을 진행할 것
...
8. GitHub 푸시 상태 — 상품 색상 자유 편집·다크모드 지퍼 발광까지 포함해서 `d2f87ce`까지 origin/main과 동기화 완료(사용자가 직접 커밋·푸시함). **사진 편집 도구(`works/index.html`)와 독일어 번역(`i18n.js`) 관련 변경 중 아직 커밋 안 된 게 남아있는지는 새 세션 시작 시 `git status`로 다시 확인할 것**(세션마다 상황이 바뀔 수 있음)
...
| **상품 색상 · 사이즈** | 색상 팔레트가 더 이상 코드에 고정돼 있지 않음 — `product_colors` 테이블(마이그레이션 `010`, 아직 미실행)로 옮겨서 Works 상품 등록/수정 폼에서 관리자가 자유롭게 추가·수정·삭제 가능(이름 + 선택적 독일어 이름 + 색상 값). 서버가 없거나 테이블이 없으면 기존 6종(블랙·화이트·핑크·딥블루·스카이블루·클레이)으로 자동 폴백. 새로 추가한 색상은 `customizer.html`/`product.html` 등 색상을 쓰는 모든 화면에 바로 반영됨. 사이즈는 여전히 전 상품 XS·S·M·L·XL 고정 + 관리자 패널에서 사이즈별 체크박스로 품절 처리. 기존에 카키·옐로우·포레스트·차콜 색상으로 촬영해뒀던 사진들은 옛 팔레트라 여전히 안 맞음(재촬영 필요) — **Supabase `products` 테이블의 실제 색상 데이터는 아직 반영 안 됨**, 아래 "지금 막혀 있는 것" 참고 |
