...
11. **마이그레이션 `022`·`023` 실행 필요** — 2026-08-30(2차)에 추가로 만든 것들:
    `022_order_device.sql`(기기별 실제 매출 집계), `023_qna_templates.sql`(CS 빠른 답변
    템플릿). 둘 다 미실행 상태에서도 사이트는 정상 동작하지만(자동 폴백, 로컬에서 직접
    재현해 확인함), 실제 효과(기기별 매출 표시·템플릿 저장)를 보려면 Supabase SQL
    Editor에서 실행 필요 — 위 "Supabase 마이그레이션" 표 참고.
...
| `022_order_device.sql` | 기기별 실제 매출 집계용 `orders.device`·`pending_payments.device` 컬럼 | ⚠️ **미실행** — 미실행 시에도 주문·결제는 정상 동작(device 없이 자동 폴백, 로컬에서 직접 재현해 정상 폴백 확인함), 대시보드 "기기별 실제 매출"만 전부 "확인 안 됨"으로 표시됨 |
| `023_qna_templates.sql` | CS 빠른 답변 템플릿 저장용 `qna_templates` 테이블 | ⚠️ **미실행** — 미실행 시에도 QnA 답변 작성엔 지장 없음(빈 템플릿 목록으로 조용히 폴백), 템플릿 추가만 안 됨 |
