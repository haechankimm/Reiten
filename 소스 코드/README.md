...
0. **⚠️ `server/migrations/010_product_colors.sql` 마이그레이션을 Supabase에서 실행할 것** — 2026-08-13에 "관리자 패널에서 상품 색상을 자유롭게 추가·수정·삭제하고 싶다"는 요청으로 색상 팔레트(`black`/`white`/`pink`/`deepblue`/`sky`/`clay` 6종 고정)를 DB 테이블(`product_colors`)로 옮기는 작업을 구현·커밋함(아직 마이그레이션은 미실행). **이 마이그레이션을 실행하기 전까지는** `GET /api/colors`가 기존 6종 정적 목록으로 자동 폴백해서 사이트 자체는 그대로 동작하지만, Works "상품" 탭에서 새 색상을 추가·수정·삭제하려고 하면 `product_colors` 테이블이 없어서 실패한다(다른 마이그레이션처럼 실행하면 즉시 해결). 실행 후 확인할 것: ① Works 상품 등록/수정 폼의 "컬러" 칸 밑에 새로 생긴 "색상 팔레트 관리"에서 색상 추가(이름 + 선택적 독일어 이름 + 색상 값) → 저장 → `customizer.html`/`product.html`에 스와치가 바로 뜨는지 ② Deutsch 모드에서 방금 만든 색상이 입력한 독일어 이름으로(안 채웠으면 한글로 폴백) 보이는지 ③ 이미 어떤 상품이 쓰는 색상은 삭제가 차단되는지
...
8. GitHub 푸시 상태 — `server.js`의 사진 업로드 버그 수정과 번역 관련 커밋들은 이미 origin에 푸시 완료(`f63eaad`까지 동기화됨, 사용자가 세션 중간중간 직접 커밋·푸시함). **사진 편집 도구(`works/index.html`)와 이번에 추가된 독일어 번역(`i18n.js`), README, 그리고 이번 세션의 상품 색상 자유 편집·다크모드 지퍼 발광 작업(아래 "최근 작업 이력" 참고)은 아직 커밋 전** — 새 세션 시작 시 `git status`로 확인 후 커밋·푸시할 것
