...
> 2026-08-14 세션(5차, 최신)에서 카드사(NHN KCP) 심사 관련 작업을 마무리했습니다 — 라이브
> 전환 직후 카드결제가 전부 막혀 있던 심각한 버그 발견·수정·배포·검증, "신용카드 결제수단이
> 안 보인다"는 심사 피드백에 대응(라디오 버튼→큰 칩 버튼 교체), 번역 버그 2건 수정 + 사이트
> 전체 번역 누락 감사(고객 화면 기준 0건 확인), 카드결제 반영해 약관·개인정보처리방침 갱신,
> Works 상품 탭 일괄 처리 기능 신규 추가까지 전부 커밋·푸시 완료(사용자가 직접 진행, working
> tree clean 확인됨). 아래는 그 결과를 반영한 새 우선순위입니다.

1. **카드사(NHN KCP) 심사 신청 마무리** — 기술적으로 막혀있던 것(결제창이 아예 안 뜨던 버그,
   신용카드 옵션이 눈에 안 띄던 문제)은 전부 고쳐서 배포·검증까지 끝났습니다(`reiten.kr`에서
   실제 라이브 KCP 결제창이 뜨는 것, 약관 동의 화면·카드사 리스트까지 정상 노출되는 것을
   스크린샷으로 확인함 — 아래 "최근 작업 이력" 참고). 남은 것: ① **본인 카드로 실제 결제
   1건을 끝까지 완료**해서(지금까지는 결제창이 뜨는 것까지만 확인했고 실제 승인·주문생성·
   웹훅까지 도는 것은 아직 검증 안 됨) 전 구간이 문제없는지 확인 ② 상품 사진 채우기 —
   2026-08-14 최종 확인 기준 `active` 상품 9개 중 사진 있는 게 2개뿐(`reflect-heart-hoodie`,
   `tshirt`)이라 카드사 심사의 "사이트 100% 완성" 항목에 걸릴 수 있음, 최소 3~5개까지 채우고
   나머지는 `active=false` 임시 비공개 권장 ③ 룩북은 8칸 전부(0/8) 여전히 사진 없음 — 같이
   정리 필요 ④ 그 다음 NHN KCP 카드사 등록신청 제출(또는 재제출 — 이미 한 번 반려됐다면
   "결제창·번역 등 수정 완료됐으니 재확인 부탁드린다"고 요청). KG이니시스·카카오페이는 아직
   상태 변화 확인 안 됨(사용자가 직접 확인 필요).
2. **마이그레이션 018 실행** — 상품 사진마다 컬러를 태그해서 컬러 클릭 시 그 사진으로 바뀌게
   하는 기능을 새로 만들었는데, 이걸 실제로 쓰려면
   `server/migrations/018_product_image_colors.sql`(`products.image_colors` 컬럼 추가)을
   Supabase에서 실행해야 합니다. **실행 안 해도 사이트가 깨지지는 않습니다.**
3. **마이그레이션 016(`orders.status` CHECK 제약) 재확인** — REST API로는 제약 존재 여부를
   읽을 방법이 없어(직접 Postgres 접속 정보가 `.env`에 없음) 아직 확인 못 했습니다. 대신
   `orders.status`에 허용된 5개 값 밖의 데이터가 없다는 것만 확인했으니 실행해도 안전합니다.
   Supabase SQL Editor에서 `server/migrations/016_orders_status_check.sql`을 다시 붙여넣어
   실행하세요 — 이미 적용돼 있다면 "constraint ... already exists" 에러가 뜰 뿐이라 그 자체가
   확인이 되고, 안 돼 있었다면 이번에 적용됩니다.
4. **재입고 발주 알림 기능을 실제 장기 품절 데이터로 검증** — 구현은 끝났지만(`server.js`의
   `checkRestockNeeded()`, 매주 월요일 09:00 실행), 조건을 충족하는 실제 데이터로 발동해본
   적은 없습니다. 급하면 서버 콘솔에서 `checkRestockNeeded()`를 수동으로 한 번 호출해 메일이
   정상 발송되는지 확인해보세요.
5. (선택) **가상계좌 결제 · 카카오 알림톡 연동** — 둘 다 우리 쪽 코드만으로는 못 끝내고 외부
...
| **미입금 자동취소** | 고객 화면 문구("3일 내 미입금 시 자동 취소")를 실제로 수행하는 크론(`node-cron`, 매시 정각) 추가 — 3일 지난 "입금대기" 주문을 자동으로 "취소" 처리하고 재고를 복원, 고객에게 안내 메일 발송. **주의**: 이 패치 이전에 만들어진 주문(items에 productId가 안 저장된 옛 주문)은 자동으로는 재고 복원이 안 되고 취소 처리만 됨 |
...
| `016_orders_status_check.sql` | `orders.status` CHECK 제약 | ⚠️ 재확인 필요 — REST API로는 제약 존재 여부를 읽을 수 없어 이번엔 확인 못 함(위 "다음 세션이 가장 먼저 할 일" 3번 참고). 기존 데이터가 전부 허용값 안에 있다는 것만 확인함(2026-08-14) |
| `017_login_lockout.sql` | 관리자 로그인 실패 잠금용 `login_attempts` 테이블 | ✅ 실행 완료(실제 7회 실패→잠금 테스트까지 통과) |
| `018_product_image_colors.sql` | 상품 사진마다 컬러 태그를 붙이는 `products.image_colors` 컬럼 | ⚠️ 미실행 — 실행 전까지는 컬러 클릭 시 사진이 안 바뀌는 기존 동작 그대로 유지됨(하위호환, 사이트가 깨지지는 않음) |
...
- **정산 자동화** — 매달 1일, 지난달 매출·환불·쿠폰 사용 내역을 정리한 파일을 관리자 이메일로 자동 발송(세무사 전달용). 기존 크론 패턴(자동취소)·CSV/엑셀 생성 로직(`orderExport.js`)·이메일(`mailer.js`)을 그대로 재사용하면 됨
- **CS 응답 초안** — Q&A 답변창(Works) 옆에 자주 쓰는 답변 템플릿을 클릭 한 번으로 채워 넣는 기능. 완전 자동응답은 아니고 답변 시간만 줄여줌
...
- **2026-08-14** — 번역 버그 2건 발견·수정 + 사이트 전체 번역 누락 감사 + 카드결제 반영해
  약관·개인정보처리방침 검토(아직 커밋 전). **① "무통장입금" 번역 안 됨** — 사용자가 실제로
  신고 → `i18n.js` 사전 키가 "무통장 입금"(띄어쓰기 있음)인데 `cart.html`의 결제수단 버튼은
  "무통장입금"(띄어쓰기 없음)을 쓰고 있어서 `t()`가 못 찾고 항상 한글 그대로 노출되던 것이었음
  — 버튼 쪽을 사전 키와 맞춰 수정. **② 상품명이 영어·일본어 모드에서도 한글로 뜨는 문제** —
  `t(p.nameKo)`로 상품명을 "번역"하려던 게 원인. `t()`는 미리 정해둔 정적 UI 문구 사전에서만
  찾기 때문에, 상품마다 계속 바뀌는 상품명은 절대 사전에 없어서 항상 한글 원문이 그대로
  나왔음(재고/상품명을 새로 입력해도 마찬가지). **관리자가 상품마다 번역을 추가로 입력할
  필요 없이** 이미 필수로 입력받는 영문명(`name`) 필드로 자동 대체하는 `productName(p)` 헬퍼를
  `app.js`에 신설(`getLang()==="ko" ? p.nameKo : (p.name || p.nameKo)`, 일본어 전용 이름 칸은
  없어서 일본어도 영문명으로 대체) — `cart.html`/`product.html`/`reviews.html`/`qna.html`/
  `app.js`의 상품명 표시 9곳을 전부 이걸로 교체. reviews.html·qna.html에 같은 이름의 로컬
  함수가 이미 있어서 이름 충돌이 나길래 `productLabel`로 개명. 실제로 영어 모드에서
  "리플렉트 하트 후디"가 "Reflect Heart Hoodie"로, "무통장입금"이 "Bank Transfer"로 뜨는 것을
  직접 확인함. **③ 사이트 전체 번역 누락 감사** — 정규식 기반 스크립트로 먼저 훑었더니 40여
  건이 누락된 것처럼 나왔는데, 번역 값(en/ja)에 `{n}`/`{name}` 같은 중괄호가 들어있으면
  정규식이 엔트리 경계를 잘못 잡는 버그가 있어서 대부분 오탐이었음 — Node `vm` 모듈로 실제
  `i18n.js`를 그대로 실행해 딕셔너리 객체를 읽는 방식으로 다시 검증(가장 신뢰도 높은 방법).
  실제로 빠진 건 카드결제 도입 이후 번역을 안 채운 `cart.html`/`order-complete.html`의 결제
  관련 문구 9개, `account.html`의 로그인 관련 문구 3개(독일어만 있고 영어·일본어 없었음) —
  전부 채움. 이 감사 스크립트로 전체 재검증한 결과 고객 화면 번역 누락 0건 확인. **④ 카드결제
  추가에 따른 약관·개인정보처리방침 검토** — `terms.html` 제8조(결제방법)가 "무통장 입금
  방식으로만 진행됩니다"로 카드결제 도입 이전 상태 그대로였음 → 신용카드 결제 문구 추가.
  `privacy.html` 제1조도 같은 이유로 수정, 제5조(개인정보 처리위탁) 표에 포트원(PortOne)이
  빠져 있어서(카드결제 시 이름·연락처·이메일이 실제로 전달됨에도) 추가함. **법률 자문이
  아니므로 실제 운영 전 검토를 권장함** — README 0번 섹션에도 이미 있는 문구.
- **2026-08-14** — 결제수단 선택 UI를 라디오 버튼에서 큰 칩 버튼으로 교체 + 체크아웃 사소한
  손질(아직 커밋 전, 위 항목의 후속). 사용자가 실제 스크린샷으로 라디오 버튼이 너무 작아서
  결제수단을 안 고르고(기본값 무통장입금 그대로) 그냥 제출해버리기 쉽다고 지적 — NHN KCP
  심사에서 "신용카드가 없다"고 느낀 것도 결국 이 작은 라디오를 놓친 것일 수 있다고 판단해,
  상품 사이즈 선택과 같은 큰 칩 버튼(`무통장입금`/`신용카드`, 52px 높이)으로 교체함. 고르는
  즉시 결제수단이 바뀌는 게 아니라 "제출" 버튼을 눌러야 하는 흐름은 그대로 유지(라디오→칩만
  바뀜). `payMethod()`가 라디오 대신 `aria-pressed` 상태를 읽도록 수정, `pay-method-field`가
  보이기 시작하는 시점에 `syncPayMethodUI()`를 명시적으로 한 번 호출하도록 추가(버튼 문구가
  항상 현재 선택과 맞게 그려지도록 보강). 쿠폰 코드 입력칸의 "있으시면 입력해 주세요"
  placeholder 문구도 사용자 요청으로 제거(문구가 없어도 위에 "쿠폰 코드" 라벨이 있어 기능
  이해에는 지장 없음). 로컬 정적 서버로 칩 클릭 시 버튼 문구·안내 박스·입금자명 칸이 전부
  올바르게 전환되는 것까지 확인함.
- **2026-08-14** — NHN KCP 카드사 심사 피드백("결제수단에 신용카드 없음") 대응(아직 커밋
  전). 카드사 등록신청 후 "주문/결제 단계에 신용카드 결제수단이 없다"는 피드백을 받음 →
  `reiten.kr`에서 다시 직접 재현: **지금은 정상적으로 결제수단 선택란에 뜨고, 결제창도
  신용카드 탭·약관 동의·카드사 리스트(삼성/신한/KB Pay/하나/롯데/농협 등)까지 전부 정상
  노출됨**을 스크린샷으로 확인함 — 즉 이 피드백은 이전 세션에서 고친 `paymentId` 40자 제한
  버그(결제창 자체가 안 열리던 문제)가 **배포되기 전에 심사원이 테스트했을 때 받은 피드백일
  가능성이 높음**(그때는 실제로 결제수단을 선택해도 오류만 나서 "없다"고 느꼈을 수 있음).
  다만 라벨이 정확히 "신용카드"가 아니라 "카드결제"로 돼 있어 혹시 모를 오해를 없애려고
  `cart.html`의 결제수단 선택란·안내 문구를 "신용카드"로 명칭 통일(`i18n.js`에 en/ja
  번역도 추가). **재수정 후 재심사 요청 시 이 최신 상태로 다시 확인해달라고 안내하면 됨.**
- **2026-08-14** — Works 상품 탭에 일괄 처리 기능 신규 추가(아직 커밋 전). 상품이 몇 개 안
  될 땐 하나씩 눌러도 되지만, 카드사 심사 전 사진 없는 상품을 몰아서 숨긴다든지, 새 컬러를
  여러 상품에 한 번에 추가한다든지 하는 작업을 위해 신설. **① 상품 탭**: 카드 우상단에 선택
  체크박스 + 상단 "전체 선택"을 추가하고, 1개 이상 선택하면 상단에 일괄 처리 바(공개 처리·
  비공개 처리·컬러 추가·선택 삭제)가 뜸. 서버에 새 엔드포인트 3개 추가
  (`PATCH /api/admin/products/bulk-active`, `PATCH /api/admin/products/bulk-color`,
  `DELETE /api/admin/products/bulk`) — 상품 하나씩 API를 반복 호출하는 대신 한 번에 처리해서
  활동 로그에도 (재고 일괄저장 때와 같은 이유로) 실행 1회당 1줄만 남도록 함. 컬러 추가는
  이미 그 컬러를 갖고 있는 상품은 건드리지 않고 없는 상품에만 추가(중복 방지). **② 재고
  탭**: 각 상품 카드에 "전체 적용" 입력칸을 추가 — 숫자를 넣고 누르면 지금 고른 컬러의 모든
  사이즈가 한 번에 그 값으로 바뀜(재입고 시 사이즈마다 같은 수량 넣기, 완전 품절 처리 시
  전부 0으로 등에 사용). 기존 재고 일괄저장 파이프라인(저장 버튼 눌러야 실제 반영)을 그대로
  타도록 구현해서 새 로직을 따로 만들지 않음. 목업 데이터로 상품 선택→공개/비공개/컬러추가/
  삭제 API가 각각 정확히 한 번씩, 올바른 ids로 호출되는 것과 재고 "전체 적용"이 사이즈 3개를
  한 번에 0으로 바꾸고 저장 버튼에 "3건"으로 반영되는 것까지 확인함.
- **2026-08-14** — ✅ 위 카드결제 수정 **배포 확인 + 결제수단 전체 재점검**. `reiten.kr`에서
  다시 조회해보니 커밋(`c129f75`)이 이미 GitHub에 푸시되고 Render 배포까지 끝나 있었음(사용자가
  직접 진행) — `/api/payments/prepare`가 36자 paymentId를 내려주는 것, `cart.html`의 안내
  문구가 "제출하면 주문서가 바로 전송됩니다"로 바뀐 것, 무통장입금 안내가 "24시간"으로 바뀐
  것까지 라이브에서 재확인함. 이어서 사용자 요청으로 **다른 결제수단·페이지에도 같은 종류의
  버그(정적 사이트 시절 가정이 서버 도입 후에도 안 고쳐진 것)가 더 있는지 점검**: ①
  `orderName`(결제창에 뜨는 상품명)도 `paymentId`처럼 KCP가 길이를 제한하는지 실제로 100자
  넣고 결제창을 열어봐서 확인 — 문제없이 열림(추가 수정 불필요) ② `no-server-notice`류
  패턴(서버 없을 때만 보여야 하는 안내)이 있는 나머지 3개 파일(`account.html`/`qna.html`/
  `reviews.html`)을 전부 다시 읽어봤는데, 셋 다 실제 서버 응답 성공 여부로 정확히 판단하고
  있어 문제없음(`cart.html`의 `pay-note`만 예외적으로 `formEndpoint` 유무만 보고 서버 자체
  유무는 확인 안 하고 있었던 것 — 이미 위에서 고침) ③ `order-lookup.html`은 애초에 서버
  없이는 동작할 수 없는 기능이라 폴백 자체가 불필요, 확인 결과 실제로 없음. **결론: 이번에
  찾은 것 외의 추가 버그는 발견되지 않음.** 다만 카드사 심사 신청 전 남은 것은 여전히 있음 —
  아래 "다음 세션이 가장 먼저 할 일" 참고(실제 완결 결제 테스트 미실시, 상품 사진 2/9·룩북
  0/8만 채워짐).
- **2026-08-14** — 🚨 라이브 전환 후 카드결제 전면 장애 발견·수정 + 체크아웃 안내 문구 버그
  + 무통장입금 자동취소 24시간 단축. **① 카드결제
  전면 장애** — 사용자가 "카드결제 누르면 결제 중 오류가 발생했습니다"라고 보고 →
  `reiten.kr`에서 실제로 로그인 없이 `/api/payments/prepare` → `PortOne.requestPayment()`를
  직접 호출해 재현: `paymentId`(`"reiten-" + crypto.randomUUID()`, 43자)가 NHN KCP V2
  라이브 채널의 40자 제한을 넘어서 결제창이 열리기도 전에 매번 실패하고 있었음(테스트
  채널에서는 이 제약이 안 걸려서 라이브 전환 전까지 발견되지 않았던 문제 — 즉 라이브 전환
  직후부터 지금까지 카드결제가 100% 막혀 있었던 것으로 추정됨). `server.js`에서 접두어를
  빼고 UUID만 쓰도록 수정(36자) → 같은 방식으로 재현해서 실제 라이브 KCP 결제창
  (`checkout-service.prod.iamport.co`)이 정상적으로 뜨는 것까지 확인함(카드번호는 입력하지
  않고 바로 닫아 실제 결제는 발생시키지 않음). **② 체크아웃 페이지 안내 문구 오류** —
  `cart.html`의 결제수단 안내(`#pay-note`)가 `SITE.order.formEndpoint` 설정 여부만 보고
  "복사해서 이메일/카카오로 보내주세요"를 표시하고 있었음 — 이건 서버 없는 정적 배포 전용
  문구인데, 이 사이트는 서버가 떠서 `/api/order`가 실제로 자동 처리하는데도 실제 고객에게
  그대로 노출되고 있었음(사용자가 `reiten.kr`에서 직접 확인). `/api/config` 성공 여부로
  서버 존재를 판단하는 `hasServer` 플래그를 추가해 문구를 정정. **③ 무통장입금 자동취소
  3일→24시간 단축** — 사용자 요청으로 `PENDING_CANCEL_DAYS=3`(일 단위)를
  `PENDING_CANCEL_HOURS=24`(시간 단위)로 변경, 고객 이메일 알림은 이미 있던 기능이라
  본문 문구만 "3일"→"24시간"으로 정정(`sendCustomerAutoCancelled`). 관련 고객 문구
  4곳(`cart.html`/`order-complete.html`/`terms.html`/`i18n.js`)도 전부 동기화. **참고**:
  `terms.html`(이용약관) 제8조가 여전히 "무통장입금 방식으로만 진행됩니다"라고만 돼 있어
  카드결제 도입이 반영 안 된 상태 — 이번엔 24시간 문구만 고치고 이 부분은 손대지 않음(법률
  문서 내용 변경은 별도 검토 필요하다고 판단)
- **2026-08-14** — 카드사 심사 대비: NHN KCP 라이브 채널 전환 + 상품 사진 버그 4건 수정
  (아직 커밋 전). **① 라이브 채널 전환** — 사용자가 포트원 콘솔에서 NHN KCP 실연동 채널을
  등록(인증서·PG상점아이디 IPA4E 등록)하고 Render의 `PORTONE_CHANNEL_KEY`를 그 채널 키로
  교체 → `https://reiten.kr/api/config`로 실제 라이브 채널 키가 반영된 것 확인함(코드 변경
  없음, 순수 설정값 교체). **② 컬러 클릭 시 상품 사진 전환** — `product.html`에서 컬러를
  클릭해도 사진이 안 바뀌던 문제(예전에 컬러 순서=사진 순서라고 가정했다가 순서가 안 맞으면
  엉뚱한 사진으로 바뀌는 사고가 있어 2026-08-09에 아예 뺐던 기능)를 다시 넣되, 이번엔 관리자가
  Works 상품 등록 폼에서 사진마다 "이 사진의 컬러"를 명시적으로 골라 태그하는 방식으로 구현해
  같은 문제가 재발하지 않게 함(`products.image_colors` 컬럼 신설, `018_product_image_colors.sql`
  — **Supabase에서 아직 실행 안 하면 이 기능은 계속 꺼진 채로 안전하게 폴백됨**). 태그 안 된
  사진은 예전처럼 그대로 유지(하위호환). **③ 사진 편집기에서 축소 가능하게** — 상품·룩북 사진
  업로드 시 뜨는 편집기가 확대(100~300%)만 가능하고 축소는 안 돼서 원본 비율을 다 못 살리는
  사진은 억지로 크롭해야 했음 → 슬라이더 최소값을 40%로 낮추고, 프레임보다 사진이 작아져
  생기는 빈 자리는 흰 배경으로 채우도록 수정(안 채우면 JPEG로 저장할 때 검은색으로 바뀜).
  캔버스 픽셀을 직접 읽어 40% 축소 시 모서리가 흰색(255,255,255)으로 채워지는 것까지 확인함.
  **④ Works 색상 삭제 버그 수정** — "클레이" 색상을 삭제하려 하면 "삭제 가능 여부를 확인하지
  못했습니다" 에러만 뜨던 문제 재현 → 원인은 `supabase-js`의 `.contains("colors", [key])`가
  jsonb 컬럼인 `colors`를 Postgres 배열 리터럴(`{clay}`)로 잘못 직렬화해서 매번 쿼리 자체가
  깨졌던 것(`invalid input syntax for type json`). `.filter("colors", "cs", JSON.stringify([key]))`
  로 바꿔서 해결 — 고치고 나니 실제로 클레이가 활성 상품 2개(`core-zip-hoodie`,
  `reflect-crop-hoodie`)에서 쓰이고 있어 "사용 중이라 삭제 불가"로 정상 차단되는 것까지 확인함.
  **⑤ 재고 탭 "이력 보기" 팝업 정리** — 관리자 이메일이 풀로 표시돼 카드 테두리를 넘어가고,
  이력이 쌓이면(최대 50건) 카드가 한없이 길어지던 문제 → 이메일은 `@` 앞부분만 보여주고
  (`emailName()`, 전체는 `title` 툴팁), 목록에 `max-height:280px`+스크롤을 줘서 카드 높이가
  이력 개수와 무관하게 항상 일정하게 유지되도록 수정. 목업 데이터로 20건을 넣고 실제로 목록이
  스크롤되는 것, 이메일이 잘려 보이는 것까지 확인함.

- **2026-08-14** — Works 전체를 "즉시 저장"에서 "고치고 저장 버튼" 방식으로 통일 + 카드사
  심사(NHN KCP) 요구사항 실사(아직 커밋 전). **① 즉시저장 → 명시적 저장으로 전환**: 재고
  칸을 고칠 때마다, 주문 상태를 고를 때마다, 반품 상태를 고를 때마다 바로 서버에 저장되던
  방식이 필드 하나 고칠 때마다 토스트가 뜨고 활동 로그에도 항목 수만큼 따로 쌓여 지저분하다는
  피드백 → 세 곳 다 "메모리에 모아뒀다가 저장 버튼 한 번" 방식으로 바꿈. 재고 탭은 상단에
  "변경사항 저장 (N건)" 버튼을 새로 추가해 여러 칸을 고친 뒤 한 번에 보내고(새 엔드포인트
  `PATCH /api/admin/inventory/bulk`, 재고 변동 이력(`inventory_log`)은 항목별로 그대로 남기되
  활동 로그(`admin_audit_log`)는 저장 1회당 1줄(`inventory.bulk_update`)만 남도록 함), 주문
  상세 패널은 상태 드롭다운을 기존 "배송정보 저장" 버튼에 합쳐 한 번에 저장, 반품 카드는
  상태 드롭다운 옆에 "저장" 버튼을 새로 달아 클릭해야 반영되게 함(리뷰·QnA·상품·룩북 등
  나머지 탭은 원래부터 버튼 클릭으로만 저장돼서 손댈 필요 없었음). 로그인 없이 목업 데이터로
  재고 탭 2건을 고친 뒤 저장 버튼을 눌러 실제로 서버 요청이 저장 시점에 딱 1번만(항목 2개를
  한 번에 담아) 나가는 것, 반품 탭에 저장 버튼이 정상적으로 뜨는 것까지 확인함.
  **② 독일어 모드 날짜 표기 재신고 확인**: 리뷰 카드·활동 로그 한 줄의 날짜 자체(`fmtDate`/
  `fmtDateTime`)는 2026-08-13에 이미 `de-DE` 형식으로 고쳐져 있어(`"14.8.2026"`) 정상 동작을
  실제로 재확인함 — 문제는 그게 아니라 반품·QnA·리뷰·활동로그 탭의 **기간 검색 필터**
  (`<input type="date">`, 예: `#rv-from`/`#al-from`)로, 이건 브라우저 자체가 제공하는 네이티브
  달력 UI라 표시 형식이 페이지의 `lang` 속성이 아니라 **브라우저 자체 언어 설정**을 따른다는
  걸 확인함(브라우저 언어가 한국어면 우리 페이지를 독일어로 바꿔도 이 필드만 "연도. 월. 일."로
  남음) — 이건 코드로 고칠 수 없는 브라우저 표준 동작의 한계라, 네이티브 달력 대신 직접 만든
  입력칸으로 바꾸는 큰 변경 없이는 해결이 안 됨(실사용자인 독일 동업자는 자기 브라우저 언어가
  독일어일 테니 실제로는 문제없이 독일어로 보일 가능성이 높음). ③ **비회원 구매 기능 확인** —
  이미 있음(`/api/order`가 `optionalAuth`라 로그인 없이도 주문 생성 가능, `cart.html`에 로그인
  강제 코드 없음, `order-lookup.html`로 비회원 주문 조회도 이미 지원) — 별도 구현 불필요.
  ④ **NHN KCP 카드사 심사 체크리스트 7개 항목을 실제 데이터로 대조**: 사용자가 심사 신청 시
  받는 질문지를 그대로 붙여줘서 Supabase를 직접 조회해 항목별로 확인함 —
  (1) 사이트 접속 가능: 외부 확인 필요라 판단 보류
  (2) **사이트 100% 완성: 아니오** — `active` 상품 9개 중 사진 있는 건 1개뿐(나머지 8개 "사진
  준비중"), 룩북 8칸 전부 사진 없음(2026-08-14 오전에 이미 확인한 내용과 동일)
  (3) 사업자 정보 6종: 전부 노출 중이나 "대표자명" 표기가 "대표 김해찬"(공백)이라 요구된 형식
  "대표:홍길동"(콜론)과 달라 **`app.js`의 `renderFooter()`를 콜론 형식으로 수정함**(`대표:${ceo}`)
  — 전화번호는 휴대폰(010-9399-6861)만 등록돼 있어 "유선전화번호" 요건과 정확히 일치하는지는
  불확실(솔직히 이 부분은 심사 기준을 제가 단정할 수 없어 확답 못 함)
  (4) 상품 가격·설명: Supabase 직접 조회로 전 상품 49,000~129,000원(0원/1원/500만원+ 없음)·
  설명 채워진 것 확인, 다만 `tshirt` 상품만 설명이 극히 부실(4자)하고 사진도 0장이라 심사 전
  임시로 비공개(`active=false`) 처리하는 걸 권장(직접 바꾸지 않고 권장만 함 — 상품 노출 여부는
  운영 판단이라 임의로 안 건드림)
  (5) **NHN KCP 라이브 결제창: 아니오(현재 테스트 채널)** — `PORTONE_CHANNEL_KEY`가 아직
  테스트 채널 키라, 심사 신청 전에 포트원 콘솔에서 라이브 채널을 등록해 키를 교체해야 함(위
  "다음 세션이 가장 먼저 할 일" 참고) — **이 항목이 지금 가장 큰 걸림돌**
  (6) 노출 금액과 결제창 금액 일치: 서버가 결제창을 띄우기 직전에 가격을 다시 계산해서 내려주는
  구조(`priceItem()`/`shippingFor()`, 11번 참고)라 설계상 항상 일치 — 별도 확인 불필요
  (7) 비회원 구매: 위 ③에서 확인한 대로 이미 가능
- **2026-08-14** — Works "재고" 탭 컬러 칩 방식으로 재설계(아직 커밋 전, 위 그리드 수정의
  후속). 그리드로 바꾼 뒤에도 상품마다 컬러가 1~4개로 달라 카드 높이가 들쭉날쭉해 지저분해
  보인다는 재신고(사용자가 실제 works.reiten.kr 스크린샷으로 확인) — 컬러별로 사이즈 줄을
  전부 펼쳐 보여주던 `.inv-color-row` 방식을 버리고, 컬러를 칩(`.inv-color-chip`)으로 골라
  그 컬러의 사이즈 한 줄(`.inv-card-sizes`)만 보여주는 방식으로 바꿔 모든 카드가 비슷한
  높이를 갖게 함. "펼쳐보지 않아도 한눈에 훑기"가 안 되는 손해를 보완하려고, 그 컬러 안에
  품절(빨강)·재고 2개 이하(주황) 사이즈가 하나라도 있으면 칩 위에 작은 점을 표시
  (`invColorStatusClass()`). 컬러 칩을 클릭하면 서버를 다시 부르지 않고 이미 받아둔
  `qtyByProduct`에서 그 컬러 몫만 다시 그리며(`invSizeRowHTML()`), 입력값 저장 시 메모리의
  `qtyByColorSize`도 같이 갱신해 컬러를 오갈 때 값이 유실되지 않게 함. 로그인 없이
  `adminFetch`를 목업으로 바꿔 컬러 수·재고 수량이 다른 샘플 상품 4개로 칩 클릭 시 값이
  올바르게 전환되는 것, 카드 높이가 균일해지는 것까지 스크린샷으로 확인함
- **2026-08-14** — Works "재고" 탭 넓은 화면 레이아웃 수정. 사용자가 스크린샷으로
  신고 — PC의 넓은 창에서는 상품 카드가 세로로 한 줄씩만 쌓여서(`display:flex;flex-direction:
  column`) 오른쪽 공간이 계속 비어 보이는 문제. 반품·QnA·리뷰 탭이 이미 쓰고 있던 반응형 카드
  그리드 패턴(`grid-template-columns:repeat(auto-fill,minmax(380px,1fr))`)으로 통일해, 화면이
  넓으면 카드가 자동으로 여러 열로 흐르고 좁으면(모바일) 자연스럽게 1열로 줄어들게 수정
  (`works/index.html`의 `paintAdminInventory()`). 실제 데이터 없이도 검증할 수 있도록 로그인
  없이 `adminFetch`를 목업으로 바꿔 8개 샘플 상품을 그려보는 방식으로 확인 — 1920px 화면에서
  4열로, 좁은 화면에서 다시 1열로 줄어드는 것까지 스크린샷으로 확인함
- **2026-08-14** — "다음 세션이 가장 먼저 할 일" 목록 3건 처리(아직 커밋 전). ① **`account.html`의
  옛 관리자 패널 삭제** — `#admin-panel` 섹션(주문/반품/재고/Q&A/상품/리뷰/룩북 관리 폼 전체,
  HTML 180줄 + JS 780여 줄)과 그 전용 헬퍼(`adminFetch` 안의 CRUD 함수들, `ORDER_STATUSES`/
  `RETURN_STATUSES`, 상품 폼용 `loadColors()` 호출)를 통째로 제거. 관리자 로그인 시각 표시
  (배지·"(Admin)" 인사말·초록 테마·로밍 말 애니메이션)는 `works.reiten.kr`과 무관한 별개
  기능이라 그대로 남기되, `/api/admin/orders` 호출이 실제로 성공했을 때만 켜지는 검증 로직
  (`tryShowAdminPanel`)은 그대로 유지. 정적 서버로 직접 렌더링해 콘솔 에러 없이 정상 동작하는
  것, 55개 서버 테스트 전부 통과하는 것까지 확인함 ② **마이그레이션 015·016 재확인** —
  Supabase REST API(service role key)로 직접 조회해 015(`return_requests.refunded` 컬럼)는
  실행된 것을 확인함. 016(CHECK 제약)은 PostgREST로는 제약 존재 여부를 읽을 수 없어(Postgres
  직접 접속 정보가 `.env`에 없어서 `pg_constraint` 조회 불가) 확인하지 못했고, 실제 데이터를
  변형하는 테스트(잘못된 status로 insert해보기)는 운영 DB라 하지 않음 — 대신 기존
  `orders.status`가 전부 허용값 안에 있는 것만 확인함(위 "Supabase 마이그레이션" 표, "다음
  세션이 가장 먼저 할 일" 1번 참고) ③ **재입고 발주 알림 신규 구현** — 아래 항목 참고.
- **2026-08-14** — 재입고 발주 알림 기능 신규 구현(아직 커밋 전, 위 항목과 같은 세션).
  `inventory_log`(재고 변동 이력, `012_inventory_log.sql`)를 최신순으로 훑으며 각 상품·컬러·
  사이즈 조합의 현재 재고(`qty`)에서 거꾸로 델타를 빼나가 재고가 마지막으로 0 이하로 떨어진
  시점을 역산하는 `findOutOfStockSince()`를 추가하고, 그 시점이 `RESTOCK_ALERT_DAYS`(7일) 이상
  지난 조합만 모아 관리자에게 메일로 보내는 `checkRestockNeeded()`를 `server.js`에 추가함(재고가
  0인 순간 즉시 가는 기존 `sendAdminLowStock`과는 별개 — 이건 그게 방치되는 경우만 잡는 용도).
  같은 알림을 매일 반복 발송하면 스팸이 되므로 일부러 매주 월요일 09:00 한 번만 실행되도록
  `cron.schedule("0 9 * * 1", ...)`으로 등록(기존 미입금 자동취소 크론과 같은 패턴).
  `server/lib/mailer.js`에 `sendAdminRestockAlert(items, thresholdDays)` 추가. 서버 문법 검사·
  부팅 테스트·기존 55개 테스트 전부 정상 확인했지만, **실제로 7일 이상 품절이 지속되는 데이터로
  발동해본 적은 없음**(위 "지금 막혀 있는 것 → 자동화 아이디어" 참고).
- **2026-08-14** — `[hidden]` CSS 버그 발견·수정(`fe0aaa3`) — `.btn`/`.chips`/`.summary__row`처럼 `display:flex`를 지정하는 클래스에 `hidden` 속성을 같이 쓰면, 브라우저 기본 규칙(`[hidden]{display:none}`)보다 그 클래스 규칙이 이겨서 hidden이어도 그대로 보이는 문제를 발견함(works/index.html은 이미 이 문제로 전역 규칙을 갖고 있었는데 고객 화면 `style.css`엔 없었음). 실제 증상 3가지 확인: ① `account.html` "비밀번호를 잊으셨나요?" 클릭 시 로그인/회원가입 탭이 재설정 폼 위에 계속 남음 ② 상품/룩북 편집 폼의 "편집 취소" 버튼이 새로 추가하는 중에도 계속 보임 ③ 장바구니 "쿠폰 할인" 요약 행이 쿠폰 없이도 항상 표시됨(이번 세션에 새로 생긴 버그). `style.css`에 works와 같은 전역 `[hidden]{display:none!important}` 규칙 추가하고, 개별로 있던 `.summary__row[hidden]`/`.lang-menu[hidden]`/`.admin-badge[hidden]`은 중복이라 제거. 실제 서버 띄워서 재설정 폼·쿠폰 화면 둘 다 고쳐진 것 확인함
- **2026-08-14** — og:image 1200×630 크롭 + 관리자 로그인 7회 실패 잠금 추가(`403351f`) — ① `product.html` OG 이미지에 Cloudinary `c_fill,g_auto` 크롭 적용(세로 4:5 상품 사진을 카카오톡·페이스북 카드 비율에 맞게 자르되 `g_auto`로 그래픽이 안 잘리게) ② 관리자 로그인 실패 잠금(같은 이메일 7회 연속 실패 시 15분) 추가 — 로그인은 브라우저가 Supabase Auth를 직접 호출해서 서버가 결과를 못 보는 구조라, `works/index.html`이 시도 전/후로 새 엔드포인트(`/api/admin/login-lock`)에 결과를 보고하는 방식으로 구현(`017_login_lockout.sql`). **⚠️ 이건 실제 로그인 화면을 통한 시도에 대한 보완장치일 뿐, Supabase Auth REST API를 anon key로 직접 두드리는 공격까지는 못 막음** — 그건 Supabase 대시보드 Authentication → Rate Limits/Attack Protection에서 별도 설정 필요(코드로는 우회 불가능한 영역). 실제로 7회 실패→잠금→성공 시 초기화까지 API 직접 호출로 테스트해서 확인함
- **2026-08-14** — 반품 자동환불·상품 JSON-LD·반응형 이미지·결제/쿠폰 통합테스트·주문상태 제약 추가(`a139f6b`) — ① 반품이 "완료"로 처음 바뀌면 카드결제 주문은 포트원 환불을 자동 시도, 실패 시 관리자에게 긴급 메일, 무통장입금은 "직접 환불 필요"로 안내(`015_return_refund.sql`) ② `product.html`에 `Product` 구조화 데이터(JSON-LD) 추가 — 컬러별 실재고까지 반영한 재고상태를 검색엔진에 노출 ③ Cloudinary 사진에 너비별 `srcset` 추가(카드 400/600/900, 상세 500~1800, 썸네일 100/160) — 기기와 무관하게 항상 원본 크기를 내려받던 문제 해결 ④ `resolveCoupon`을 `server.js`에서 `server/lib/coupons.js`로 분리해 Supabase 없이 테스트 가능하게 만들고, 가짜 Supabase 클라이언트(`server/test-helpers/fakeSupabase.js`)로 쿠폰 유효성 11개 + "장바구니→소계→배송비→할인→결제금액" 통합 시나리오 7개 테스트 추가(총 55개 테스트 통과) ⑤ `orders.status`에 DB CHECK 제약 추가(`016_orders_status_check.sql`, 오타로 잘못된 상태값이 조용히 들어가는 것 방지)
- **2026-08-14** — 상품 등록 폼에 "실제 페이지에서 보기" 링크 추가(`4bdce85`) — 저장 직후 또는 기존 상품 수정 화면을 열 때 그 상품의 실제 `product.html`을 새 탭으로 바로 열어볼 수 있는 링크를 저장 버튼 옆에 추가. 사진 비율 버그처럼 "관리자 미리보기와 실제 사이트가 다르게 보이는" 문제를 저장한 그 자리에서 바로 확인하기 위함
- **2026-08-13** — 상품 상세 페이지 카카오톡·SNS 공유 미리보기(OG 태그) 추가(`ab61299`) — `product.html`은 정적 파일이라 `og:title`/`og:image`를 JS로 넣어도 카카오톡·페이스북 링크 미리보기 봇(JS를 실행하지 않고 서버가 준 raw HTML만 읽음)에는 반영되지 않았음 → `express.static`보다 먼저 `GET /product.html?id=...` 요청만 가로채는 라우트를 추가해 `<head>`의 title·description·og 태그를 실제 상품 정보로 바꿔치기해서 반환(id 없거나 상품 못 찾으면 그대로 정적 파일로 폴백)
- **2026-08-13** — 상품 사진 업로드 크롭 비율을 정사각(1:1)에서 4:5로 수정(`dfb0840`) — 관리자 상품 등록 폼의 사진 편집 도구가 1:1로 고정돼 있었는데, 실제 쇼핑몰(카드·상세 페이지)은 전부 4:5 세로 비율로 사진을 보여줘서, 관리자 미리보기(1:1 박스)에서는 정상으로 보여도 실제 사이트에 올라가면 비율이 안 맞아 이상하게 보였음(사용자가 실제 화면에서 발견해 신고). 크롭 프레임과 관리자 쪽 미리보기 박스를 전부 4:5로 통일. **이미 1:1로 업로드된 기존 사진은 자동으로 안 고쳐짐**(잘린 픽셀은 복구 불가) — 해당 상품들은 사진을 다시 선택해 4:5로 재업로드해야 함. **`account.html`의 중복 관리자 폼에는 이 수정이 반영 안 돼 있음**(위 "다음 세션이 가장 먼저 할 일" 1번 참고)
- **2026-08-13** — 재고를 상품+사이즈에서 상품+컬러+사이즈 단위로 세분화(`ff149a7`) — 사용자 확인 하에(마이그레이션 실행 즉시 기존 재고가 컬러 구분이 없어 전부 초기화된다는 점 동의받음) `014_inventory_by_color.sql` 적용. `decrement_inventory`/`restore_inventory` RPC를 컬러까지 확인하도록 재정의, `withRealSoldOut()`이 컬러별로 계산해 상품 응답에 `outOfStockByColor` 추가, `product.html`은 지금 고른 컬러 기준으로만 사이즈 품절을 판정하고 컬러 변경 시 선택된 사이즈가 품절이면 자동으로 다른 사이즈로 전환. Works 재고 탭을 상품×컬러×사이즈 그리드로 재구성(아직 값이 없는 조합도 바로 입력 가능), CSV/엑셀 내보내기·재고 이력에도 컬러 컬럼 추가. 실 서버로 색상 전환 시 사이즈 가용성이 정확히 바뀌는 것, 완전 품절 시 "품절" 버튼 비활성까지 실제 데이터로 확인함
- **2026-08-13** — 반품 자동환불·재고 세분화 전, 미입금 자동취소·재고 이력·쿠폰 시스템·카카오 골격 추가(`8463a39`) — ① 매시 정각, 3일 지난 "입금대기" 주문을 자동 취소 + 재고 복원 + 고객 안내메일(`node-cron`, `011_auto_cancel_and_restock.sql`) ② 반품 카드에 "재고 복원" 버튼 추가(주문 저장 시 productId/size/color를 함께 남기도록 바꿔서 복원 대상 파악 가능) ③ 재고 변동 이력(`inventory_log`, `012_inventory_log.sql`) — 주문차감/자동취소복원/반품복원/관리자수정 구분 기록, Works 재고 탭에 상품별 이력 조회 UI 추가 ④ 카드결제 재고부족 취소 시도까지 실패하면(이중 실패) 관리자에게 즉시 긴급 이메일 ⑤ 쿠폰(할인코드) 시스템 신규 구축(`013_coupons.sql`) — Works에 쿠폰 탭 추가(정률/정액, 전체/지정상품, 최소주문금액, 사용한도, 기간, 활성화 자유 설정), 장바구니에 쿠폰 코드 입력·적용 UI, 주문 생성 시 서버가 재검증 후 할인 반영 ⑥ 카카오 알림톡 발송 골격 추가(`server/lib/kakao.js`) — 발송대행사 연동 전까지는 안전하게 아무 것도 안 함(이메일은 정상 동작)
- **2026-08-13** — Works 대규모 기능 확장(`fe0aaa3` 이전 커밋들) — ① 반품·QnA·리뷰 탭에도 주문 탭과 같은 검색어·상태·기간 필터 추가 ② 상품 등록 폼 컬러 칩에 실제 색상 동그라미 표시(그동안 "색상 팔레트 관리" 목록엔 동그라미가 있었는데 정작 상품에 쓸 색 고르는 칩은 글자만 있었음) ③ 매출 대시보드·재고 목록도 CSV/엑셀 내보내기 추가 ④ 고객 화면에 localStorage 기반 "최근 본 상품" 섹션 추가(상품 상세 페이지) ⑤ 위 기능에 쓰인 신규 텍스트 번역 보강(관리자: 독일어, 고객화면: 영어·일본어)
- **2026-08-13** — 전 페이지 버그 점검(아직 커밋 전) — 16개 고객 페이지 + Works를 콘솔 에러·모바일 화면 기준으로 훑어봄. 대부분 이상 없었고, `cart.html`에서 실제 버그 1개 발견·수정: 좁은 화면에서 수량 조절칸(`.qty`)이 자리를 다 차지하면 옆의 "삭제" 버튼이 남은 폭에 맞춰 글자가 한 자씩 세로로 쪼개져 보이던 문제 → `.line-item__foot`에 `flex-wrap: wrap`을 줘서 공간이 부족하면 삭제 버튼이 통째로 다음 줄로 내려가게, `.remove`엔 `white-space: nowrap`을 줘서 버튼 자체는 항상 한 줄로 유지되게 수정. 나머지 페이지는 특별한 문제를 못 찾음(발견 즉시 고친 재고 탭/활동 로그/룩북/햄버거 버그는 이미 이전 커밋에 반영됨)
- **2026-08-13** — 사용자가 실기기 스크린샷으로 신고한 버그 2개 수정(아직 커밋 전). ① Works 모바일 햄버거 버튼이 빈 사각형으로만 보이던 문제 — 다른 아이콘 버튼(`.icon-btn`)엔 `svg` 크기를 지정하는 CSS가 있었는데 새로 만든 `.sidebar-toggle`엔 그게 빠져 있어서 SVG가 브라우저 기본 크기로 렌더링돼 버튼 밖으로 밀려나 있었음 → `.sidebar-toggle svg { width:18px; height:18px }` 추가 ② 룩북(`lookbook.html`, 홈 티저 공용 `.look` 클래스) 모바일 화면에서 사진 비율이 이상해 보이던 문제 — 실제 원인은 각 칸의 비율 자체는 정확했는데(`aspect-ratio`로 계산해서 확인함), CSS 그리드 기본 정렬(`align-items: stretch`)이 같은 줄에서 비율이 다른 칸을 서로 같은 높이로 늘려서 짧은 비율(예: 16:10) 칸 밑에 빈 공간이 크게 생겼던 것 → `.look`에 `align-items: start` 추가로 각 칸이 자기 비율대로만 높이를 갖게 수정(모바일뿐 아니라 데스크톱에도 있던 문제였음, 로컬에서 실제 픽셀 값 찍어서 원인 확인 후 수정)
...
- **2026-08-13** — **상품 색상 팔레트를 관리자 패널에서 자유롭게 추가·수정·삭제 가능하게 변경**(`d2f87ce`, 커밋·푸시·`reiten.kr`/`works.reiten.kr` 실배포 확인까지 완료) + **다크모드 지퍼 발광 추가**. ① 지금까지 `black`/`white`/`pink`/`deepblue`/`sky`/`clay` 6종으로 코드에 고정돼 있던 컬러를 `product_colors` 테이블로 옮김(`server/migrations/010_product_colors.sql`, Supabase에 실행 완료). `GET /api/colors`가 공개 목록을, `GET/POST/PATCH/DELETE /api/admin/colors`가 관리자 CRUD를 담당(패턴은 `/api/admin/settings`를 따름). 이미 어떤 활성 상품이 쓰는 색상은 삭제를 막음(`products.colors`에 포함되는지 확인 후 409) ② 색상을 추가할 때 **독일어 이름은 선택 입력** — Works 관리자 패널이 한국어/Deutsch를 지원하는데, 관리자가 코드 밖에서 자유 입력한 색상 이름은 `i18n.js` 딕셔너리에 없어서 그대로 두면 독일어 화면에서도 한글로 노출됨(기존 폴백 규칙과 동일) → 입력 폼에 독일어 이름 칸을 추가해 그 자리에서 해결, 비워두면 한글로 폴백 ③ Works 상품 등록/수정 폼의 "컬러" 칸 아래 "색상 팔레트 관리" UI 신설(추가/수정/삭제), 새로 만든 색상은 `customizer.html`/`product.html` 등 컬러를 쓰는 모든 화면에 즉시 반영(`app.js`의 `loadColors()`가 `loadProducts()`와 같은 패턴으로 서버에서 최신 목록을 받아옴) ④ 상품 저장 시 서버 쪽 색상 검증(`server/lib/products.js`의 `productPatchFromBody`)이 코드에 고정된 정적 목록만 허용하고 있던 걸 발견·수정 — 이걸 못 고쳤으면 관리자가 새로 만든 색상을 상품에 골라 저장해도 서버가 "모르는 색상"이라며 조용히 지워버렸을 것 ⑤ **다크모드(스튜디오 라이트 스위치)에서 참·소매 로고만 발광하고 지퍼 금속은 그냥 어두워지기만 하던 것**을 참과 같은 마감 규칙(실버=강하게·컬러=은은하게·매트블랙=무광 유지)으로 지퍼도 발광하도록 `hoodie.js`에 추가(`zipperGlowClass()`, 원단색 지퍼 테이프는 옷감과 같이 계속 어두워짐 — 금속 부분만 발광). 실제 브라우저에서 세 마감 전부 스크린샷으로 확인함. 참고로 코드 확인 결과 지퍼 무광 동작은 최초 커밋부터 있던 것이라 최근 회귀는 아니었음 ⑥ 검증 중 Supabase `products` 테이블에 아직 옛 색상(카키·차콜 등)이 남아있는 상품에서 홈/상품상세/스튜디오가 흰 화면으로 죽는 **기존 버그를 우연히 발견해서 방어 코드도 같이 넣음**(위 "상품 데이터 정리" 항목 참고) ⑦ 이 과정에서 Works·옛 관리자 패널(`account.html`)의 폼 "취소" 버튼이 독일어 모드에서 주문 상태 "취소"(Cancelled/Storniert)와 같은 번역 키를 공유해서 엉뚱하게 "Storniert"로 뜨던 기존 버그도 같이 발견·수정(`data-i18n`을 별도 키로 분리, 화면에 보이는 한글은 그대로 "취소")
...
| 정산 자동화 · CS 응답 초안 | 2026-08-14에 논의, "별도 프로그램 대신 지금 서버(크론)·Works에 기능으로 얹는다"로 방향만 정함 — 0번 섹션 "자동화 아이디어" 참고 |
