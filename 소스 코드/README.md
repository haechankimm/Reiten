...
> 2026-08-29 세션(6차, 최신)에서 카카오 로그인, Works 모바일 UI 수정, 정산 자동화, 그리고
> GA4·채널톡(CS 채팅)·네이버 서치어드바이저 연동 + Works 대시보드 "방문자 분석" 섹션까지
> 구현했습니다. 코드는 전부 커밋됐지만(`카카오로그인`·`패치`·`정산 자동화`·GA4 관련 커밋)
> **GitHub push는 사용자가 직접 GitHub Desktop으로 해야 하는 부분이라 완료 여부 미확인**입니다.
> 아래는 그 결과를 반영한 새 우선순위입니다 — 1~3번이 이번 세션에서 새로 생긴 마무리 작업,
> 4번부터는 이전 세션(2026-08-14)부터 이어져 온 항목입니다.
...
2. **GA4(Google Analytics) 방문자 분석 마무리** — 코드는 이미 다 만들어져 있고
   (`server/lib/analytics.js`, Works "대시보드" 탭 하단 "방문자 분석" 섹션), 딱 두 가지만
   남았습니다: ① `analytics.google.com` → 관리 → 속성 설정에서 **속성 ID**(숫자) 확인 ②
   같은 관리 화면의 **"속성 액세스 관리"**에 서비스 계정 이메일
   (`reiten@reiten-507009.iam.gserviceaccount.com`)이 **"뷰어"** 권한으로 등록돼 있는지 확인
   (등록 여부가 이번 세션 중 확인 안 됨). 이 둘이 준비되면 Render 환경변수에
   `GA4_SERVICE_ACCOUNT_JSON`(로컬 `server/.env`에 이미 있는 값 그대로 복사)과
   `GA4_PROPERTY_ID`(위에서 확인한 속성 ID) 두 개를 추가하면 끝 — 미설정 상태에서도 사이트는
   정상 동작하고 그 섹션만 안 뜹니다(선택 기능).
3. **채널톡(CS 채팅) · 네이버 서치어드바이저 실사이트 확인** — 코드는 16개 페이지에 전부 넣고
   로컬 검증까지 끝냈지만, 배포된 `reiten.kr`에서 실제로 채팅 버튼이 뜨는지, 네이버 서치
   어드바이저 "소유확인" 버튼이 성공하는지는 배포 후 확인이 필요합니다(1번 push가 선행돼야 함).
4. **카드사(NHN KCP) 심사 — 등록 완료 확인됨(2026-08-29), 실결제 테스트만 남음** — NHN KCP
...
5. **마이그레이션 018 실행** — 상품 사진마다 컬러를 태그해서 컬러 클릭 시 그 사진으로 바뀌게
...
6. **마이그레이션 016(`orders.status` CHECK 제약) 재확인** — REST API로는 제약 존재 여부를
