...
> `charms.html`·`reviews.html`·`cart.html`·`order-complete.html`·`order-lookup.html`·`account.html`·`return-request.html`·`qna.html`은 활성 메뉴가 없습니다(`renderHeader("")`).
...
| `PRODUCTS` | 상품 8종 |
| `CHARM_PRICE` | 참 가격 (5,900원) — 한 곳에서 관리 |
...
| 엔드포인트 | 인증 | 설명 |
|---|---|---|
| `POST /api/order` | 선택(로그인 시 주문에 연결) | `{ customer, items }`을 받아 `productId`/`charm`/`extras`만으로 서버에서 가격을 재계산, `decrement_inventory` RPC로 재고를 원자적으로 차감(부족하면 `409 OUT_OF_STOCK`), `orders` 테이블에 저장, Resend로 **관리자 알림 메일 + 고객 주문접수 확인 메일**을 각각 발송(둘 다 실패해도 주문은 성공 처리) |
...
```js
{
  ...
  colors: ["black", "cream", "sky"],               // COLORS의 키만 사용
  sizes: ["S", "M", "L", "XL"],
...
- [x] 빌드 없는 정적 사이트 (10페이지) + 선택적 Node 백엔드
...
- [x] 로그인 시 본인 주문내역 조회
- [x] **관리자(role=admin) 로그인 시에만 열리는 숨김 패널** — 전체 주문 조회/상태 변경, 반품·교환 신청 조회/상태 변경, 재고 수량 조회/수정, Q&A 답변. UI를 숨기는 것과 별개로 서버가 매 요청마다 권한을 재검증
...
| 항목 | 현재 상태 | 해야 할 일 |
|---|---|---|
| **사업자 정보** | `[상호 입력]` 등 자리표시자 | 사업자등록 · 통신판매업 신고 후 `data.js`의 `SITE.biz` 채우기 |
| **입금 계좌** | `[은행명 입력]` 자리표시자 | `SITE.order` 채우기 |
| **개인정보처리방침 / 이용약관** | About에 요약만 | 전문 페이지 별도 작성 필요 (`privacy.html`, `terms.html`) |
| 연락처 · 채널 | 자리표시자 | 이메일 · 카카오 채널 · 인스타 계정 |

> ⚠️ 통신판매업 신고 전, 또는 사업자 정보 미표기 상태로 판매하면 **전자상거래법 위반**입니다.
> 반드시 신고를 마치고 정보를 채운 뒤 공개하세요.
...
- [ ] `sitemap.xml`, `robots.txt` 추가 (charms.html/reviews.html 포함)
