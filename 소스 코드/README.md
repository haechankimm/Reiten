...
1. **GitHub push + Render 배포 확인** — 이번 세션에서 만든 커밋들이 로컬에는 있지만 push
   여부가 확인 안 됐습니다. GitHub Desktop 열어서 Push origin 클릭 → Render **Events** 탭에서
   최신 커밋이 "Deploy live"로 떴는지 확인하세요.
2. **GA4·채널톡·네이버 서치어드바이저 실사이트 확인** — 코드는 16개 페이지에 전부 넣고 로컬
   검증까지 끝냈지만(GA4는 실제 속성 ID로 데이터 수신까지 확인함), 배포된 `reiten.kr`에서
   실제로 채팅 버튼이 뜨는지, 네이버 서치어드바이저 "소유확인" 버튼이 성공하는지는 배포 후
   확인이 필요합니다(1번 push가 선행돼야 함). GA4는 Render 환경변수 2개
   (`GA4_SERVICE_ACCOUNT_JSON`, `GA4_PROPERTY_ID` — 로컬 `server/.env`에 이미 있는 값 그대로
   복사)를 추가해야 Works 대시보드에도 뜹니다.
...
9. **시스템 개선 아이디어 7개 진행** — 위 항목들과는 성격이 달라서(버그 수정이 아니라 Claude가
   ...
   따로 정리해뒀습니다. 사용자가 다음 세션에서 진행하기로 확정함 — 우선순위 순으로 그 섹션
   참고.
...
> 다음 세션에서 우선순위대로 진행하면 됨 — 순서만 정해뒀고 아직 아무것도 시작 안 함.

*바로 가치 있는 것(적은 노력 대비 효과 큼)*
1. **알림센터에 "시스템 오류" 카테고리 추가** — 카드결제 이중실패·환불 실패·웹훅 실패 같은
   급한 이벤트가 지금은 관리자 이메일로만 가서, 이메일을 놓치면 그냥 놓친다. 이미 있는
   알림센터(`GET /api/admin/notifications`)에 이 카테고리만 추가하면 Works 들어오자마자
   바로 보이게 됨
2. **알림센터 자동 갱신(폴링)** — 지금은 로그인 시점에 한 번만 조회해서, Works를 계속 켜두고
   있으면 그새 들어온 신규 주문을 새로고침 전까지 모름. 60초 정도 폴링 추가로 보완 가능
3. **CSP(콘텐츠 보안 정책) 켜기** — `server.js`가 `helmet({contentSecurityPolicy: false})`로
   완전히 꺼둔 상태. 이번 세션에 GA4·채널톡·카카오 로그인 등 외부 스크립트가 확 늘어난 지금이
   정리해서 켤 시점 — XSS 방어에 실질적 도움. 켜려면 각 외부 스크립트 도메인을 허용목록에
   넣는 작업이 필요함
