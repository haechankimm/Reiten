...
1. **⚠️🔴 상품 등록 실패 버그 — 재현·원인 파악에 사용자의 정확한 에러 메시지가 필요함** — 2026-08-13에 관리자 패널(Works)에서 "상품 등록"이 실패한다는 리포트를 받았는데, 코드 리뷰(`productPatchFromBody`, `POST /api/admin/products`)와 Supabase에 직접 테스트 insert를 해봐도 재현이 안 됨(정상 동작 확인됨). **다음 세션에서 가장 먼저 할 일: 다시 실패할 때 뜨는 토스트 메시지 원문(또는 브라우저 개발자도구 Console/Network 탭 스크린샷)을 받아서 진단할 것.** 그 정보 없이는 추측성 수정만 반복하게 됨
...
7. GitHub 푸시 상태는 이 커밋(`2b4f103`)까지 이미 origin과 동기화됨(카드결제 기능 포함) — 새 세션 시작 시 `git status`로 로컬 미커밋 변경만 확인하면 됨(이번 세션의 Works 독일어 번역·통화 표기 수정은 아직 커밋 전)
