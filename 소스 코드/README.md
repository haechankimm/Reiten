...
| **사업자 정보** | `assets/js/data.js`의 `SITE.biz` 전부 채움(상호 Reiten · 대표 김해찬 · 사업자등록번호 154-44-01222 · 통신판매업신고 제-2026-용인기흥-00638 호 · 주소 · 전화번호 010-9399-6861). **인스타그램 계정(`reiten_korea`) 반영 완료**(2026-08-13, 푸터에 실제 프로필로 연결되는 링크로 노출). **카카오톡 채널명("Reiten")도 반영**했지만 `SITE.order.kakaoUrl`(실제 채널 링크, `https://pf.kakao.com/_...`)은 아직 자리표시자 — 채워지면 이름이 자동으로 클릭 가능한 링크로 바뀌고, 그 전까지는 지금처럼 이름만 텍스트로 노출됨(`app.js`의 `renderFooter()`) |
...
**오픈 전 확인**
- `SITE.order.kakaoUrl`(실제 카카오톡 채널 URL, `https://pf.kakao.com/_...`) 채우기 — 채널명("Reiten")은 이미 반영됨, URL만 남음(인스타그램은 2026-08-13에 완료)
...
- **2026-08-13** — 카카오톡 채널 이름 반영(아직 커밋 전) — `SITE.order.kakao`를 실제 채널명 "Reiten"으로 채움. 링크로 만들려면 채널 URL이 따로 필요해서 `SITE.order.kakaoUrl` 필드를 새로 추가하고, 거기 실제 `https://pf.kakao.com/_...` 주소가 채워지면 자동으로 클릭 가능한 링크가 되도록 `renderFooter()`를 고쳐둠(지금은 URL이 자리표시자라 이름만 텍스트로 노출). 사용자가 URL을 알려주면 바로 연결 예정
...
| 카카오톡 채널 · 인스타그램 계정 | 인스타그램 ✅ 완료(`reiten_korea`, 2026-08-13). 카카오톡 🟡 이름만 완료("Reiten") — `SITE.order.kakaoUrl`(실제 채널 링크)이 아직 자리표시자라 지금은 텍스트로만 보이고 링크는 안 걸림 |
...
- [ ] `data.js`의 `SITE.order.kakaoUrl` 자리표시자 교체(실제 카카오톡 채널 URL, 채널명 "Reiten"은 이미 반영됨)
