...
> 발견·수정**했습니다(아래 1번). 재고 탭 즉시저장→저장버튼 전환, 상품 사진-컬러 연결,
> 무통장입금 자동취소 24시간 단축도 같이 처리했습니다. 아래는 그 결과를 반영한 새 우선순위.

1. **🚨 배포 필요 — 카드결제가 실제로는 전부 막혀 있었습니다.** 사용자가 "카드결제 누르면
   결제 중 오류가 발생했습니다"라고 보고 → `reiten.kr`에서 직접 재현해 원인을 찾음: 결제
   요청에 쓰는 `paymentId`가 `"reiten-" + crypto.randomUUID()`(43자)였는데, **NHN KCP V2
   라이브 채널은 주문번호(paymentId)를 40자까지만 허용**해서 `PortOne.requestPayment()`가
   결제창을 열기도 전에 매번 실패하고 있었습니다(포트원 테스트 채널에선 이 제약이 안 걸려서
   지금까지 못 보고 넘어감). `server.js`에서 접두어를 빼고 UUID(36자)만 쓰도록 수정 →
   실제 라이브 KCP 결제창(`checkout-service.prod.iamport.co`)이 정상적으로 뜨는 것까지
   `reiten.kr`에서 직접 재현해 확인했습니다(카드 정보는 입력 안 하고 바로 닫음). **이건
   아직 커밋만 됐고 Render에 배포 전입니다 — 배포해야 실제로 고쳐집니다.**
...
3. **카드사(NHN KCP) 심사 신청 마무리** — 라이브 채널 전환 자체는 완료됐고(위 1번 배포되면
   실제로 결제가 됩니다), 남은 것: ① 1번 배포 후 **본인 카드로 실제 소액 결제 1건**을 끝까지
   진행해서 확인 ② 상품 사진 채우기 — `active` 상품 9개 중 사진 있는 게 소수라(2026-08-14
   재확인 시점 기준 tshirt도 채워짐, 나머지는 재확인 필요) 카드사 심사의 "사이트 100% 완성"
   항목에 걸릴 수 있음, 최소 3~5개 상품에 사진 채우고 나머지는 `active=false` 임시 비공개
   권장 ③ 룩북도 사진 채우기 필요 ④ 사업자 정보 "대표" 표기는 콜론 형식(`대표:김해찬`)으로
   이미 고쳐서 배포 대기 중 ⑤ 그 다음 NHN KCP 카드사 등록신청 제출. KG이니시스·카카오페이는
   아직 상태 변화 확인 안 됨(사용자가 직접 확인 필요).
...
- **2026-08-14** — 🚨 라이브 전환 후 카드결제 전면 장애 발견·수정 + 체크아웃 안내 문구 버그
  + 무통장입금 자동취소 24시간 단축(아직 커밋만 됨, **Render 배포 필요**). **① 카드결제
