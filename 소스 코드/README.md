...
> 2026-08-14 세션(3차)에서 카드사 심사 준비를 실제로 진행했습니다 — 포트원에 NHN KCP
> 라이브 채널을 등록하고 `PORTONE_CHANNEL_KEY`를 라이브 키로 교체 완료(`/api/config`로
> 확인됨). 그리고 상품 사진·컬러 관련 버그 4건도 같이 고쳤습니다. 아래는 그 결과를 반영한
> 새 우선순위입니다.

1. **마이그레이션 018 실행 (신규, 이번 세션에 추가된 기능에 필요)** — 상품 사진마다 컬러를
   태그해서 컬러 클릭 시 그 사진으로 바뀌게 하는 기능을 새로 만들었는데, 이걸 실제로 쓰려면
   `server/migrations/018_product_image_colors.sql`(`products.image_colors` 컬럼 추가)을
   Supabase에서 실행해야 합니다. **실행 안 해도 사이트가 깨지지는 않습니다** — 그냥 컬러
   클릭해도 사진이 안 바뀌는 예전 상태 그대로 남을 뿐입니다.
2. **카드사(NHN KCP) 심사 신청 마무리** — 라이브 채널 전환은 **완료**됐습니다(포트원에서 NHN
   KCP 실연동 채널 등록 → `PORTONE_CHANNEL_KEY` 교체 → `/api/config`에서 새 채널 키로 바뀐
   것 확인함, 2026-08-14). 남은 것: ① **본인 카드로 실제 소액 결제 1건**을 사이트에서 끝까지
   진행해서 결제창→승인→주문생성이 정상 동작하는지 확인(아직 안 해봄) ② 상품 사진 채우기 —
   Supabase 직접 조회 결과 `active` 상품 9개 중 사진 있는 게 1개뿐이라(나머지 "사진 준비중"),
   카드사 심사 질문지의 "사이트 100% 완성" 항목에 걸림. 최소 3~5개 상품에 사진을 채우고
   나머지는 `active=false`로 임시 비공개 처리하는 걸 권장(직접 안 바꿨음 — 운영 판단이라서)
   ③ 룩북도 8칸 전부 사진이 없어서 같이 정리 필요 ④ 사업자 정보 "대표" 표기는 요구된 콜론
   형식(`대표:김해찬`)으로 이미 고쳐서 배포 대기 중 ⑤ 그 다음 NHN KCP 카드사 등록신청 제출.
   KG이니시스·카카오페이는 아직 상태 변화 확인 안 됨 — 포트원 콘솔에서 재확인 필요(Claude는
   외부 콘솔에 로그인할 수 없어 이 항목은 사용자가 직접 확인해야 합니다).
