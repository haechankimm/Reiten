...
> 2026-08-14 세션(2차)에서 이 목록에 있던 5개 항목 중 3개를 처리했습니다 — ① `account.html`의
> 옛 관리자 패널 삭제 완료 ② 마이그레이션 015는 직접 조회로 실행 확인(016은 REST API 한계로
> 확인 불가, 아래 참고) ③ 재입고 발주 알림 기능 신규 구현. 아래는 그 결과를 반영한 새 우선순위입니다.

1. **마이그레이션 016(`orders.status` CHECK 제약) 재확인** — 015는 이번 세션에 Supabase REST
   API(service role key)로 `return_requests.refunded` 컬럼이 실제로 존재하는 것을 직접 조회해
   ✅ 확인했습니다. 016은 CHECK 제약이라 REST API(PostgREST)로는 존재 여부를 읽을 방법이
   없어서(직접 Postgres 접속 정보가 `.env`에 없음) 이번엔 확인하지 못했습니다 — 대신
   `orders.status`에 허용된 5개 값(입금대기/입금확인/배송중/완료/취소) 밖의 데이터가 하나도
   없다는 것만 확인했으니 실행해도 안전합니다. Supabase SQL Editor에서
   `server/migrations/016_orders_status_check.sql`을 다시 붙여넣어 실행하세요 — 이미 적용돼
   있다면 "constraint ... already exists" 에러가 뜰 뿐이라 그 에러 자체가 "이미 됐다"는 확인이
   되고, 안 돼 있었다면 이번에 적용됩니다(둘 다 안전한 재실행).
2. **⚠️ 카드사(NHN KCP) 심사 신청 전 라이브 채널 전환 필요 — 이전 조언 정정**. 지난 세션엔
   "상품 준비될 때까지 전환은 미뤄도 된다"고 안내했는데, 2026-08-14에 사용자가 실제 카드사
   심사 신청 질문지를 확인해보니 **"NHN KCP 결제창이 테스트가 아닌 실제 결제창으로 떠야
   심사가 진행된다"는 조건이 명시돼 있어 이 조언은 틀렸습니다.** 순서: ① NHN KCP
   "카드사심사 등록현황"에서 개별 카드사 심사까지 끝났는지 확인 ② **포트원(PortOne) 콘솔**에서
   NHN KCP **라이브 채널**을 등록해 라이브 채널키 발급받기(지금 `PORTONE_CHANNEL_KEY`엔 테스트
   채널 키만 들어있음) ③ Render 환경변수의 `PORTONE_CHANNEL_KEY`를 라이브 키로 즉시 교체
   ④ 교체 직후 실제 결제 1건을 전 구간(결제창→서버검증→웹훅) 테스트해서 살아있는지 확인
   ⑤ 그 다음에 카드사 심사 신청(체크리스트 7개 항목 전체는 아래 "최근 작업 이력" 참고 —
   가장 큰 남은 문제는 상품 사진 미비, 다음 항목 참고). KG이니시스·카카오페이는 아직 상태
   변화 확인 안 됨 — 포트원 콘솔에서 재확인 필요(Claude는 외부 콘솔에 로그인할 수 없어 이
   항목은 사용자가 직접 확인해야 합니다).
3. **재입고 발주 알림 기능을 실제 장기 품절 데이터로 검증** — 이번 세션에 구현은 끝났지만
   (`server.js`의 `checkRestockNeeded()`, 매주 월요일 09:00 실행 — 상세는 아래 "최근 작업
   이력" 참고), 조건(재고 0인 상품·컬러·사이즈 조합이 7일 이상 지속)을 충족하는 실제 데이터로
   발동해본 적은 없습니다. 급하면 서버 콘솔에서 `checkRestockNeeded()`를 수동으로 한 번 호출해
   메일이 정상 발송되는지 확인해보세요.
4. (선택) **가상계좌 결제 · 카카오 알림톡 연동** — 둘 다 우리 쪽 코드만으로는 못 끝내고 외부
