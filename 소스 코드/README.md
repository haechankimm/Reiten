...
> 2026-08-14 세션(4차)에서 **라이브 전환 직후 카드결제가 전부 실패하던 심각한 버그를
> 발견·수정하고, 실제로 Render 배포까지 끝난 것을 라이브에서 재확인**했습니다(사용자가
> 직접 커밋·푸시·배포 진행). 결제수단 전체에 비슷한 버그가 더 있는지도 점검해서 추가 문제는
> 없는 것으로 확인됨. 아래는 그 결과를 반영한 새 우선순위입니다.

1. **카드사(NHN KCP) 심사 신청 마무리** — 결제창이 안 뜨던 버그는 고쳐서 배포·검증까지
   끝났습니다(`reiten.kr`에서 실제 라이브 KCP 결제창이 뜨는 것 확인함, 아래 "최근 작업 이력"
   참고). 남은 것: ① **본인 카드로 실제 결제 1건을 끝까지 완료**해서(지금까지는 결제창이
   뜨는 것까지만 확인했고 실제 승인·주문생성·웹훅까지 도는 것은 아직 검증 안 됨) 전 구간이
   문제없는지 확인 ② 상품 사진 채우기 — 2026-08-14 재확인 기준 `active` 상품 9개 중 사진
   있는 게 2개뿐(`reflect-heart-hoodie`, `tshirt`)이라 카드사 심사의 "사이트 100% 완성"
   항목에 걸릴 수 있음, 최소 3~5개까지 채우고 나머지는 `active=false` 임시 비공개 권장
   ③ 룩북은 8칸 전부(0/8) 여전히 사진 없음 — 같이 정리 필요 ④ 사업자 정보 "대표" 표기는
   콜론 형식(`대표:김해찬`)으로 이미 고쳐서 배포됨 ⑤ 그 다음 NHN KCP 카드사 등록신청 제출.
   KG이니시스·카카오페이는 아직 상태 변화 확인 안 됨(사용자가 직접 확인 필요).
...
| **다국어** | 고객 화면: 한국어/English/日本語(헤더에서 전환). 관리자 패널: 한국어/Deutsch(`works.reiten.kr` 상단바에서 전환 — 독일인 동업자용, 고객 화면 헤더 언어 메뉴에는 안 나옴. `account.html`에 있던 옛 언어 전환 UI는 2026-08-14 옛 관리자 패널과 함께 삭제됨). 장바구니는 담을 때 언어가 아니라 볼 때마다 현재 언어로 다시 번역되도록 수정됨. 관리자 패널을 Deutsch로 봤을 때 한글로 새던 몇몇 포인트(사진 선택 버튼, 반품·교환 신청, 룩북 "w12 (전체)" 옵션 등) 번역 보완 완료 |
...
| `016_orders_status_check.sql` | `orders.status` CHECK 제약 | ⚠️ 재확인 필요 — REST API로는 제약 존재 여부를 읽을 수 없어 이번엔 확인 못 함(위 "다음 세션이 가장 먼저 할 일" 1번 참고). 기존 데이터가 전부 허용값 안에 있다는 것만 확인함(2026-08-14) |
...
- **2026-08-14 확인(Supabase 직접 조회)**: 지금 사이트에 `active` 상태인 상품 9개 중 **사진이
  있는 건 1개(`reflect-heart-hoodie`)뿐**이고 나머지 8개는 전부 사진 0장(고객 화면엔 "사진
  준비중" 플레이스홀더로 뜸). 룩북도 8칸 전부 사진이 없어서 룩북 페이지가 사실상 빈 화면.
