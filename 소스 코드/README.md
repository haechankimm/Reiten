...
9. **시스템 개선 아이디어 7개 진행** — 위 항목들과는 성격이 달라서(버그 수정이 아니라 Claude가
   ...
   2026-08-29 다음 세션에서 구현·로컬 검증까지 완료**(아래 섹션 참고) — 중간 우선순위 3개
   (테스트 커버리지·`works/index.html` 파일 분리·Supabase 백업 정책 확인)와 아이디어 단계
   1개(재입고 발주 수량 추천)는 아직 시작 전.
...
| `019_system_error_log.sql` | 카드결제 이중실패·환불 실패를 Works 알림센터에서 바로 보여주기 위한 `system_error_log` 테이블 | ⚠️ **미실행** — 실행 전까지는 알림센터의 "시스템 오류" 항목이 조용히 0건으로만 뜸(다른 마이그레이션과 같은 원칙으로 사이트 동작 자체엔 지장 없음) |
...
1. ~~알림센터에 "시스템 오류" 카테고리 추가~~ — **구현 완료**. 카드결제 이중실패(재고부족으로
   ...
   `019` — **아직 미실행**, 위 표 참고)에 쌓아 알림센터(`GET /api/admin/notifications`)가
...
*중간 우선순위*
4. **테스트 커버리지 — 특히 최근에 추가한 다단계 로직** — 지금 있는 테스트 55개는 순수 계산
   로직(가격·쿠폰) 위주라, "주문 취소 → 재고복원 + 환불 + 메일"처럼 여러 단계가 연쇄되는
   로직(2026-08-29에 추가)은 테스트가 전혀 없음. 회귀 버그가 나기 제일 쉬운 영역이라 우선
   보강 권장
