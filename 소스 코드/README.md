...
2. **카드결제 PG 라이브 채널 승인 진행 중 — NHN KCP 쪽 구비서류 심사 완료**(2026-08-14,
   사용자가 NHN KCP 가맹점 관리 화면 스크린샷으로 확인). 아직 남은 것: ① NHN KCP 화면 왼쪽의
   "카드사심사 등록현황"에서 개별 카드사(롯데·비씨·현대·국민 등) 심사까지 전부 끝났는지 확인
   ② 끝났다면 **포트원(PortOne) 콘솔**에서 NHN KCP **라이브 채널**을 새로 등록해 라이브
   채널키를 발급받기(지금 `PORTONE_CHANNEL_KEY`엔 테스트 채널 키만 들어있음) ③ 라이브 채널로
   실제 결제 1건을 전 구간(결제창→서버검증→웹훅) 테스트 ④ **상품 사진·상세페이지가 준비되는
   시점에 맞춰** Render 환경변수의 `PORTONE_CHANNEL_KEY`를 라이브 키로 교체(무통장입금은 이미
   정상 동작 중이라 서두를 필요 없음 — 자세한 순서는 대화 로그 참고). KG이니시스·카카오페이는
   아직 상태 변화 확인 안 됨 — 포트원 콘솔에서 재확인 필요(Claude는 외부 콘솔에 로그인할 수
   없어 이 항목은 사용자가 직접 확인해야 합니다).
